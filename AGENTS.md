...
| F20 | 分群成员知识库 | `schema.sql` + `member_knowledge.py` + Agent 只读工具 + summary context + `config.py` + `README.md` + 本文件「Member knowledge invariant」 | — | 原话不复制；证据必须同群同成员；未知成员不个性化；锁定栏目不可覆盖；失败不推进游标。 |
